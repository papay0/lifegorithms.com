...
I created a `widget` for every feature that I think Robinhood was missing.  
Users will be able to go to the settings and add only the one they want.
...
👉 I built a login screen for SoTrade describing what it does, showing the Terms of Service.
...
To manage this signin on the backend, I’m using the NodeJS Framework named [robinhood-node](https://github.com/aurbano/robinhood-node).

Robinhood doesn’t have a public API, but they of course do have private APIs on [api.robinhood.com](https://api.robinhood.com/)!
...
For every request, I decrypt the token using the following lines:  
